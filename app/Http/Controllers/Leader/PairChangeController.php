...
class PairChangeController extends Controller
{
    ...
    public function store(Request $request, Pair $pair): RedirectResponse
    {
        ...
        $request->validate([
            ...
            'requested_partner_id' => ['nullable', 'exists:users,id', 'different:student_id'],
        ]);
        ...
        $existing = PairChangeRequest::where('requested_by', $student->id)
            ->where('status', 'escalated_to_admin')->exists();
        if ($existing) {
            return back()->with('error', "A pending pair change request for {$student->name} already exists.");
        }

        $requestedPartner = $request->requested_partner_id
            ? User::findOrFail($request->requested_partner_id) : null;

        $type = 'unspecified';
        if ($requestedPartner) {
            $type = $requestedPartner->halqa_id === $student->halqa_id ? 'same_halqa' : 'cross_halqa';
        }
    ...
    public function index(Request $request): Response
    {
        ...
        $reqs = PairChangeRequest::where('leader_id', $leader->id)
            ->with(['student:id,name,student_id', 'currentPartner:id,name', 'requestedPartner:id,name'])
