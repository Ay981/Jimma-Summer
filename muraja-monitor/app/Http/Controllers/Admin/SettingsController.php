...
class SettingsController extends Controller
{
    public function index(): Response
    {
        ...
        $settings = collect($keys)->mapWithKeys(fn ($k) => [$k => ProgramSetting::get($k, '')])->toArray();

        $ayat = AyatRotation::orderBy('id')->get()->map(fn ($a) => ['id' => $a->id, 'text' => $a->text, 'reference' => $a->reference])->toArray();

        $todayPrayer = PrayerTimesCache::where('date', today()->toDateString())->first();

        return Inertia::render('Admin/Settings', [
            'settings' => $settings,
            'ayat'     => $ayat,
            'prayer_today' => $todayPrayer ? ['fajr' => $todayPrayer->fajr, 'isha' => $todayPrayer->isha] : null,
        ]);
    }
    ...
    public function update(Request $request): RedirectResponse
    {
        ...
        ProgramSetting::set('exam_mode',    $request->boolean('exam_mode')    ? '1' : '0');
        ProgramSetting::set('ramadan_mode', $request->boolean('ramadan_mode') ? '1' : '0');
    ...
    public function fetchPrayerTimes(): RedirectResponse
    {
        ...
        try {
            ...
            if (!$timings) throw new \Exception('No timings in response');
            ...
            return back()->with('success', "Prayer times fetched: Fajr {$timings['Fajr']} · Isha {$timings['Isha']}");
        } catch (\Exception $e) {
            return back()->with('error', 'Could not fetch prayer times. Check your internet connection.');
        }
    }
    ...
    {
        $request->validate(['text' => ['required', 'string'], 'reference' => ['required', 'string', 'max:100']]);
        AyatRotation::create(['text' => $request->text, 'reference' => $request->reference]);
