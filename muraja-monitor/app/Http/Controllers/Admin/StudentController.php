...
class StudentController extends Controller
{
  ...
  public function index(): Response
  {
    ...
    $students = User::where("role", "student")
      ->with([
        "halqa",
        "watchlistEntries" => fn($q) => $q->whereNull("resolved_at"),
      ])
      ->orderBy("name")
      ->get();
    ...
    $subs60 = PairSubmission::whereIn("subject_student_id", $ids)
      ->where(
        "submission_date",
        ">=",
        $today->copy()->subDays(59)->toDateString(),
      )
    ...
    $totalPages = PairSubmission::whereIn("subject_student_id", $ids)
      ->selectRaw(
        "subject_student_id, COALESCE(SUM(page_to - page_from + 1), 0) as pages",
      )
      ->groupBy("subject_student_id")
      ->pluck("pages", "subject_student_id");

    // Bulk: partner map
    $allPairs = Pair::whereIn("student_a_id", $ids)
      ->orWhereIn("student_b_id", $ids)
      ->with(["studentA:id,name", "studentB:id,name"])
      ->get();
    $partnerMap = [];
    foreach ($allPairs as $pair) {
      $partnerMap[$pair->student_a_id] = $pair->studentB?->name;
      if ($pair->student_b_id) {
        $partnerMap[$pair->student_b_id] = $pair->studentA->name;
      }
    }
    ...
    $rows = $students->map(function ($student) use (
      ...
      $s60 = ($subs60[$student->id] ?? collect())->keyBy(
        fn($s) => Carbon::parse($s->submission_date)->toDateString(),
      );
      $lastSub = $lastSubDates[$student->id] ?? null;

      $sparkline = $last14Dates
        ->map(fn($d) => isset($s14[$d]) ? 1 : 0)
        ->values()
        ->toArray();
  ...
  public function show(User $student): Response
  {
    ...
    $totalPages =
      PairSubmission::where("subject_student_id", $student->id)
        ->selectRaw("COALESCE(SUM(page_to - page_from + 1), 0) as pages")
        ->value("pages") ?? 0;

    // Admin note
    $adminNote =
      AdminNote::where("student_id", $student->id)
    ...
    $onWatchlist = Watchlist::where("student_id", $student->id)
      ...
      ->exists();
    $badges = Badge::where("user_id", $student->id)
      ->get()
      ->map(
        fn($b) => [
          "type" => $b->badge_type,
  ...
  public function import(Request $request): RedirectResponse
  {
    ...
    return redirect()
      ...
      ->with(
        "success",
        "Import done: {$created} created, " . count($failed) . " skipped.",
      );
  }
  ...
  public function update(Request $request, User $student): RedirectResponse
  {
    ...
    $request->validate([
      ...
      "available_times.*" => [
        "in:after_subhi,after_zuhr,after_asr,after_maghrib,after_isha",
      ],
    ...
    $pair = Pair::where(function ($q) use ($student) {
      $q->where("student_a_id", $student->id)->orWhere(
        "student_b_id",
        $student->id,
      );
    })
      ->whereNotNull("student_b_id")
      ->first();
  ...
  public function toggleWatchlist(User $student): RedirectResponse
  {
    ...
    $entry = Watchlist::where("student_id", $student->id)
      ->whereNull("resolved_at")
      ->first();
    if ($entry) {
  ...
  {
    $ids = array_values(
      array_filter([$request->query("a"), $request->query("b")]),
    );
    ...
    $students = User::whereIn("id", $ids)
      ...
      ->map(function ($s) use ($cs, $days, $today) {
        ...
        $pages = (int) $totalSubs->sum(
          fn($r) => $r->page_to - $r->page_from + 1,
        );
        ...
        $weekly = collect(range(7, 0))
          ->map(function ($weeksAgo) use ($s, $today) {
    ...
    ContactLog::where("student_id", $student->id)
      ...
      ->each(
        fn($c) => $events->push([
          ...
          "label" =>
            "Contacted via {$c->method}" . ($c->note ? ": {$c->note}" : ""),
          "meta" => [
            "by" => $c->contactedBy?->name,
            "follow_up" => $c->follow_up_required,
          ],
        ]),
      );

    // Badges
    Badge::where("user_id", $student->id)->get()->each(
      fn($b) => $events->push([
        "type" => "badge",
        "date" => Carbon::parse($b->earned_at)->toISOString(),
        "label" =>
          "Badge earned: " . ucwords(str_replace("_", " ", $b->badge_type)),
        "meta" => [],
      ]),
    );
